...
const Feedback = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    setSubmitting(true);
    try {
      await fetch(`${process.env.NEXT_PUBLIC_API_URL}/feedback/add`, {
        method: 'POST',
  ...
  return (
    <div className="max-w-lg mx-auto my-12 p-8 bg-gradient-to-br from-amber-100 via-amber-50 to-yellow-200 dark:from-yellow-900 dark:via-amber-900 dark:to-yellow-950 rounded-3xl shadow-2xl border-2 border-amber-300 dark:border-yellow-900 relative overflow-hidden">
      ...
      <h2 className="text-2xl font-extrabold text-center text-yellow-900 dark:text-amber-100 mb-7 tracking-tight drop-shadow-lg z-10 relative">
        We Value Your Feedback
      </h2>
      <form onSubmit={handleSubmit} className="space-y-5 z-10 relative">
        <div>
          <label className="block text-base font-semibold mb-1 text-yellow-900 dark:text-yellow-100">Name</label>
          <input
            type="text"
            name="name"
            value={form.name}
        ...
        <div>
          <label className="block text-base font-semibold mb-1 text-yellow-900 dark:text-yellow-100">Email</label>
          <input
            type="email"
            name="email"
            value={form.email}
        ...
        <div>
          <label className="block text-base font-semibold mb-1 text-yellow-900 dark:text-yellow-100">Message</label>
          <textarea
            name="message"
            rows="4"
            value={form.message}
        ...
        <div>
          <label className="block text-base font-semibold mb-1 text-yellow-900 dark:text-yellow-100">Rating</label>
            ...
            {[1, 2, 3, 4, 5].map((r) => (
              <button
                ...
                className={
                  r <= form.rating
                    ? 'text-amber-500'
                    : 'text-yellow-900 dark:text-yellow-100 opacity-40'
                }
                aria-label={`Rate ${r} star${r > 1 ? 's' : ''}`}
              >
                <Star className="w-7 h-7" fill={r <= form.rating ? '#f59e42' : 'none'} />
              </button>
            ))}
          </div>
        </div>
        ...
        >
          <Send className="w-5 h-5" /> {submitting ? 'Sending...' : 'Send Feedback'}
        </button>
      </form>
    </div>
  );
